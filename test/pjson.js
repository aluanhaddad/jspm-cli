...
suite('package.json', function() {
  ...
  inputs.forEach(function(input) {
    test(input.replace(/\.json$/, ''), function() {
      ...
      test.write();
      var expected = fs.readFileSync('./test/fixtures/pjson/expected/' + input).toString().replace(/\r/g, '');
      var actual = fs.readFileSync('./test/outputs/pjson/' + input).toString().replace(/\r/g, '');
      try {
